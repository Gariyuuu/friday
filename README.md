...
See [`HANDOFF.md`](HANDOFF.md) for a cold-start onboarding guide,
[`docs/IMPLEMENTATION_PLAN.md`](docs/IMPLEMENTATION_PLAN.md) for the full build plan
and phasing, [`ARCHITECTURE.md`](ARCHITECTURE.md) for how the pieces fit
together, [`SECURITY.md`](SECURITY.md) for the threat model, and
[`PROJECT_STATE.md`](PROJECT_STATE.md) for what's actually done right now.
...
- Command palette → **Quick Actions** — real local Mac tools (open an app, open a
  ...
  permission/approval flow — the same one voice uses. See `SECURITY.md`.
...
- Live data today with **zero setup**: crypto prices (CoinGecko) and US severe
  ...
  API keys — see `.env.example` and `PROJECT_STATE.md` for exactly where to
  get them.
...
See [`PROJECT_STATE.md`](PROJECT_STATE.md) for the up-to-date phase
